            ...
            <form action="{{ route('handle.login') }}" method="POST" class="md:px-10 sm:px-5 py-16 text-center space-y-5"
                ...
                <div class="flex flex-col">
                    ...
                    <input type="text" name="email" value="{{ old('email') }}" id="email"
                        ...
                        placeholder="Enter Email" required minlength="7" maxlength="250">
                    @error('email')
                ...
                <div>
                    <button type="submit" class="btn-ascent-md w-full"><i data-feather="key"
                            class="h-4 w-4 opacity-50 absolute mr-auto"></i>Signin</button>
                </div>

                <div class="space-y-3">
                    <p class="text-slate-600 text-xs">Dosen't have an account? <a href="{{ route('view.register') }}"
                            class="font-medium text-xs text-web-ascent hover:text-web-ascent-dark">Register now</a></p>

                            <a href="{{ route('view.forgot.password') }}"
