        ...
        <div class="grid md:grid-cols-4 sm:grid-cols-1 md:gap-7 sm:gap-5">
            ...
            <div class="md:col-span-4 sm:col-span-1">
                <div class="flex md:flex-row sm:flex-col items-center md:space-x-5">
                    <img src="{{ is_null(auth()->user('admin')->profile) ? asset('admin/images/default-profile.png') : asset('storage/'.auth()->user('admin')->profile) }}" id="profile" alt="profile" class="h-24 w-24 rounded-full border bg-white" />
            ...
            <div class="input-group">   
                ...
                <input type="text" name="name" value="{{old('name',auth()->user('admin')->name)}}" class="input-box-md @error('name') input-invalid @enderror" placeholder="Enter name" required>
            ...
            <div class="flex flex-col">
                ...
                <input type="email" name="email" value="{{old('email',auth()->user('admin')->email)}}" class="input-box-md @error('email') input-invalid @enderror" placeholder="Enter email" required>
            ...
            <div class="flex flex-col">
                ...
                <input type="tel" name="phone" value="{{old('phone',auth()->user('admin')->phone)}}" class="input-box-md @error('phone') input-invalid @enderror" placeholder="Enter phone" required>
...
<script>
    ...
    const handleProfilePreview = (event) => {
        if (event.target.files.length == 0) {
            document.getElementById('profile').src = "{{ is_null(auth()->user('admin')->profile) ? asset('admin/images/default-profile.png') : asset('storage/'.auth()->user('admin')->profile) }}";
        }
        else {
