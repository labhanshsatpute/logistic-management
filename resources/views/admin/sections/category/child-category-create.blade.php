...
<div>
    <h1 class="panel-title">Add Child Category</h1>
    <ul class="breadcrumb">
        <li><a href="{{route('admin.view.dashboard')}}">Admin</a></li>
        <li><i data-feather="chevron-right"></i></li>
        <li><a href="{{route('admin.view.child.category.list')}}">Child Categories</a></li>
        <li><i data-feather="chevron-right"></i></li>
        <li><a href="{{route('admin.view.child.category.create')}}">Create</a></li>
    </ul>
</div>
@endsection

@section('panel-body')
<form action="{{route('admin.handle.child.category.create')}}" method="POST" enctype="multipart/form-data">
@csrf
<figure class="panel-card">
    ...
    <div class="panel-card-body">
        <div class="grid md:grid-cols-4 sm:grid-cols-1 md:gap-7 sm:gap-5">
            ...
            <div class="flex flex-col">
                ...
                <select name="parent_category_id" class="input-box-md @error('parent_category_id') input-invalid @enderror" required>
                    <option value="">Select Parent Category</option>
                    @foreach ($parent_categories as $parent_category)
                    <option value="{{$parent_category->id}}">{{$parent_category->name}}</option>
                    @endforeach
                </select>
                @error('parent_category_id')<span class="input-error">{{ $message }}</span>@enderror
            </div>
    ...
    <div class="panel-card-footer">
        <button type="submit" class="btn-primary-md md:w-fit sm:w-full">Add Child Category</button>
    </div>
</figure>
</form>
@endsection
